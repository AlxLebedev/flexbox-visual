export default class DescriptionGenerator {
  constructor() {
    ...
    this.flexDirectionProperty = {
      ...
      row: 'Значение row определяет, что главная ось направлена так же, как и ориентация текста, по умолчанию слева направо. Стоит отметить, что если направление текста изменено, например, свойство dir имеет значение rtl, то направление оси пойдет справа на лево.',
    ...
    this.flexWrapProperty = {
      ...
      nowrap: 'Данное значение не позволяет флекс-элементам переходить на новые строки - все они будут размещены на одной линии в направлении flex-direction, при необходимости сжаты.',
    ...
    this.flexGrowProperty = {
      common: `
      Это свойство определяет способность флекс-элемента растягиваться в случае необходимости. Оно принимает значение от нуля, которое служит пропорцией. Это свойство, какое количество доступного пространства внутри гибкого контейнера должен занимать элемент.
    ...
    this.flexShrinkProperty = {
      common: `
      Это свойство определяет способность флекс-элемента сжиматься в случае необходимости. Оно принимает значение от нуля, которое служит пропорцией.
      Ноль - это значение по умолчанию.
      `,
    };
